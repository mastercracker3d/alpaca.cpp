...
void createFolderIfNotExists(const char* carpeta) {
    if (!CreateDirectory(carpeta, NULL)) {
        // La carpeta ya puede existir o puede haber un error en la creación
        // Verifica si el error es porque la carpeta ya existe
        if (GetLastError() != ERROR_ALREADY_EXISTS) {
            fprintf(stderr, "Error al crear la carpeta: %lu\n", GetLastError());
            exit(EXIT_FAILURE);
        }
    }
}
char* getFirstFile(const char* folder) {
    ...
    snprintf(busqueda, sizeof(busqueda), _T("%s/*.txt"), folder);

    WIN32_FIND_DATA findFileData;
    HANDLE hFind = FindFirstFile(busqueda, &findFileData);
    ...
    do {
        char* localFile = findFileData.cFileName;

        if (strlen(localFile) > 3) {
            primerArchivo = strdup(localFile);
            break;  // Salir después de encontrar el primer archivo
        }
    ...
    if (name != NULL)
    {
        ...
        if (archivo != NULL) {
            ...
            while (fgets(buffer, sizeof(buffer), archivo) != NULL) {
                if (count > 0)
                {
                    sprintf(content + strlen(content), "%s", newLine);
                }
                sprintf(content + strlen(content), "%s", buffer);
                count++;
            }
            int i, j;
            int longitud = strlen(content);
            ...
            //printf("\ntextoABuscar posicion: %s\n", posicion);
            if (posicion != NULL) {
                sprintf(processedFolder + strlen(processedFolder), "%s", "/");
                sprintf(processedFolder + strlen(processedFolder), "%d", timestamp);
                //printf("\nTimestamp processedFolder: %s", processedFolder);
                sprintf(processedFolder + strlen(processedFolder), "%s", "-(");
                //printf("\nprocessedFolder guion: %s", processedFolder);
                sprintf(processedFolder + strlen(processedFolder), "%s", name);
                sprintf(processedFolder + strlen(processedFolder), "%s", ")-question_processed.txt");

                //printf("\nnuevoTexto: %s\n", processedFolder);

            }

            char nuevoNombre[255] = "";
            //printf("\nReemplazado: %s\n", processedFolder);
...
int fileResponse(long lTimestamp,char name[255], char* response)
{
    ...
    fprintf(archivo,"%s", response);
    fclose(archivo);
    completefileName[0] = '\0';
    status = 1;
    }
    return status;
}
...
//bool is_prime(int n) {)";
    ...
    if (params.interactive) {
        fprintf(stderr, "== Running in File Interchange mode. ==\n\n"
#if defined (__unix__) || (defined (__APPLE__) && defined (__MACH__)) || defined (_WIN32)
